...
public:
    ...
    void OnAttach() override
    {
        ...
        m_Camera = Camera(glm::vec3(0.0f, 12.0f, 6.0f),
                          ...
                          -90.0f, -65.0f);
        m_Camera.MovementSpeed = 0.0f;          // 相机由代码控制
...
private:
    ...
    {
        // 4 面半透明边界墙
        auto createWall = [&](const std::string& name, const glm::vec3& pos, const glm::vec3& scale) {
        ...
        createWall("Wall_North",  glm::vec3(0.0f, 0.5f, -half),  glm::vec3(half * 2.0f, 1.0f, 0.3f));
        createWall("Wall_South",  glm::vec3(0.0f, 0.5f,  half),  glm::vec3(half * 2.0f, 1.0f, 0.3f));
        createWall("Wall_West",   glm::vec3(-half, 0.5f, 0.0f),  glm::vec3(0.3f, 1.0f, half * 2.0f));
        createWall("Wall_East",   glm::vec3( half, 0.5f, 0.0f),  glm::vec3(0.3f, 1.0f, half * 2.0f));
    }
    ...
    void SetupLighting()
    {
        ...
        lightEnv.SetAmbientLight(glm::vec3(1.0f, 0.95f, 0.9f), 0.2f);
        ...
        // 顶光源
        PointLight pl;
        pl.Position  = glm::vec3(0.0f, 8.0f, 0.0f);
        ...
        // 暖色点光源
        pl.Position  = glm::vec3(0.0f, 1.5f, 0.0f);
        pl.Color     = glm::vec3(0.9f, 0.6f, 0.3f);
        pl.Intensity = 1.0f;
        pl.Range     = 12.0f;
        lightEnv.AddPointLight(pl);
    }
    ...
    void SpawnCollectible()
    {
        ...
        mesh.Material->GetProperties().EmissionStrength = 0.5f;
        ...
        script.OnUpdate = [](Entity& e, Timestep ts) {
            ...
            e.GetTransform().SetEulerAngles(euler);
            // 上下浮动
            float t = e.GetTransform().Position.y;
            // 使用 static 不是线程安全，但对游戏 demo 足够了
        };
    ...
    void SpawnEnemy()
    {
        ...
        mesh.Material->GetProperties().EmissionStrength = 0.3f;
    ...
    void CheckCollisions()
    {
        ...
        for (auto it = m_Collectibles.begin(); it != m_Collectibles.end();)
        {
            ...
            if (dist < k_CollectRadius)
            {
                ...
                SpawnParticles(gem->GetTransform().Position,
                               glm::vec4(1.0f, 0.85f, 0.1f, 1.0f));

                gem->IsActive = false;          // 隐藏
                it = m_Collectibles.erase(it);   // 移除追踪
            }
            else
            {
                ++it;
        ...
        for (auto it = m_Enemies.begin(); it != m_Enemies.end();)
        {
            ...
            {
                m_Lives--;
                SpawnParticles(enemy->GetTransform().Position,
                ...
                {
                    m_GameOver = true;
                    SpawnParticles(playerPos, glm::vec4(1.0f, 0.0f, 0.0f, 1.0f));
    ...
    void SpawnParticles(const glm::vec3& pos, const glm::vec4& color)
    {
        auto* fx = m_Scene->CreateEntity("FX_Burst_" + std::to_string(m_FxCounter++));
        fx->GetTransform().Position = pos;

        ParticleEmitterConfig config;
        config.EmitRate    = 0;          // 不连续发射
        config.BurstCount  = 20;         // 一次性爆发
    ...
    {
        glm::vec3 playerPos = m_Player->GetTransform().Position;
        glm::vec3 targetCamPos = playerPos + glm::vec3(0.0f, 12.0f, 6.0f);

        // 平滑跟随
        m_Camera.Position = glm::mix(m_Camera.Position, targetCamPos, 0.08f);
        // 确保相机看向玩家
        m_Camera.Front = glm::normalize(playerPos - m_Camera.Position + glm::vec3(0.0f, 0.5f, 0.0f));
    }
    ...
    void RenderScene()
    {
        ...
        while ((err = glGetError()) != GL_NO_ERROR)
            CORE_ERROR("[CubeEscape] GL Error: 0x{:x}", (unsigned)err);
    ...
    void RestartGame()
    {
        ...
        for (auto& e : entities)
        {
            if (e->GetTag().find("Collectible_") == 0 ||
                e->GetTag().find("Enemy_")       == 0 ||
                e->GetTag().find("FX_Burst_")    == 0)
            {
        ...
        {
            Entity* e = m_Scene->GetEntity(toRemove[i]);
